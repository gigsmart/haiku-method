...
### Changed
- **Collaborative elaboration: decision-log gate replaces turn-count gate.** Previous behavior gated stage advancement on `elaboration_turns >= 3`, which Goodhart's-law'd into agents asking padding questions to satisfy the counter. New behavior gates on `decision_log.length > 0` OR an honest `elaboration_no_decisions: true` declaration. The user's involvement is measured by *decisions where the human shaped or acknowledged the plan*, not by *count of times the agent said something*. `elaboration_turns` still increments for telemetry but no longer gates. Operationalizes H·AI·K·U as Human + AI Knowledge **Unification** — collaboration at decision points, not as ritual.

### Added
- `haiku_decision_record` MCP tool. Two modes: (a) record a real decision (`{decision, options[], choice, source: "user" | "autonomous-acknowledged", rationale?}`) which appends to the stage's `decision_log`; (b) declare `no_decisions: true` with a rationale (≥10 chars) when the stage has no architectural choices in scope. Validates `options.length >= 2`, source enum, and rationale presence. Decision-log entries become intent provenance — readers six months later can see exactly which decisions the human made vs. which the agent decided autonomously.
- Collaborative elaborate-phase prompt rewritten with quality-bar discipline: questions to the user MUST clear (a) real decision (not codebase-answerable), (b) ≥2 concrete options, (c) tradeoff axis, (d) records as a decision. Banned patterns explicitly named (yes/no defaults, codebase-answerable, permission-asking, confirmation-seeking).
- **One question at a time (NEVER batch)** — even multiple questions stay sequential because a deeper conversation on any one breaks the others. Replaces prior allowance for batched `ask_user_visual_question` calls.
- Autonomous-decision surfacing pattern — when the agent can resolve a choice from the codebase or clear conventions, it decides and surfaces the choice for veto-style approval ("I'm using X because Y; reply 'change' if not"), then records as `source: "autonomous-acknowledged"`. Most decisions in routine stages should be autonomous-acknowledged.
- Universal `interpretation: lens | strict` frontmatter field on hat-like files (hats, review-agents, fix-hats, studio-level review-agents, studio-level fix-hats). When set, the orchestrator injects a matching interpretive block right after the mandate is inlined in the dispatch prompt. **Lens** mode: mandate is the lens, in-spirit findings count, out-of-mandate findings do not. **Strict** mode: mandate is the literal checklist, findings must tie to a named item. Default (unset) preserves prior dispatch-prompt behavior — no silent migration.
- Studio uptake of the `interpretation` field: 189 review files set to `lens` (per-stage adversarial review-agents, studio-level review-agents, reviewer-style hats across every studio); 5 set to `strict` for literal-checklist mandates where false positives carry weight (security-assessment scope-compliance, compliance audit-readiness, legal contract execution formality, hwdev regulatory-coverage, hwdev DFM-readiness).
- Per-hat `run_quality_gates: true` frontmatter opt-in. When set, `haiku_unit_advance_hat` from that hat runs the unit's quality gates and auto-rejects (bolt+1, same hat retries) on failure rather than returning an error for the agent to fix-and-retry. Eliminates the agent decision point between fix-in-place and reject_hat. Bolt cap (5) still applies; exhaustion returns `max_bolts_exceeded` with reason `quality_gate_auto_reject` for human escalation.
- TDD discipline (RED → GREEN → REFACTOR) on the software studio's `builder` hat with reviewer-side reinforcement (flag implementations without preceding failing-test commits). Builder hat opts into `run_quality_gates: true` so gate failures auto-bolt.
- Superpowers-derived discipline patterns folded into FSM contract blocks: anti-placeholder rules in elaborate (TBD/similar-to/etc. reject at advance), verification-before-advance and no-hedging summaries in execute, investigate-before-edit + verify-before-fixing + two-stage closure in fix loops, scope-tightening on discovery subagents, and red-flag self-detection lists in every contract block.
