...
export function Tollbooth() {
	return (
		<motion.div
			...
			className="my-8 rounded-2xl border-2 border-cyan-400/20 bg-white p-8 text-center dark:bg-stone-900"
		>
			...
			<div className="flex items-center justify-center gap-3 border-t border-stone-200 pt-5 font-semibold text-green-500 dark:border-stone-700">
 ...
 * 1. Detected automatically during /haiku:elaborate (tooling discovery)
			...
			<div className="rounded-xl border border-violet-200 bg-violet-50/40 p-5 dark:border-violet-800 dark:bg-violet-950/10">
				...
				<div className="grid gap-2 sm:grid-cols-2">
					{[
						...
						<div
							key={row.file}
							className="flex items-start gap-2 rounded-lg border border-stone-200 bg-white p-2.5 dark:border-stone-700 dark:bg-stone-900"
						>
							<code className="mt-0.5 shrink-0 text-[0.65rem] font-bold text-amber-500">
								{row.file}
							</code>
							<span className="text-[0.65rem] text-stone-400 dark:text-stone-500">
								→ {row.gates}
							</span>
						</div>
					))}
