...
Every studio is a named lifecycle template — a DAG of stages with their own hats, review gates, and input dependencies. The default software studio has six stages, wired together by the `inputs:` declarations in each `STAGE.md`. The orchestrator won't start a stage until every stage it reads from has finished.

<ExpandableDiagram chart={`flowchart TB
    Start([Intent]) --> Inception["🎯 Inception<br/><i>review: ask</i><br/><i>collaborative</i>"]
    Inception --> Design["🎨 Design<br/><i>review: external + ask</i><br/><i>collaborative</i>"]
    Inception --> Product["📋 Product<br/><i>review: external + ask</i><br/><i>collaborative</i>"]
    Design --> Product
    Inception --> Development
    Design --> Development
    Product --> Development["🔨 Development<br/><i>review: external + ask</i><br/><i>collaborative</i>"]
    Inception --> Operations
    Product --> Operations
    Development --> Operations["🚀 Operations<br/><i>review: auto</i><br/><i>autonomous</i>"]
    Inception --> Security
    Product --> Security
    Development --> Security["🔐 Security<br/><i>review: external + ask</i><br/><i>autonomous</i>"]
    Operations --> Done([Deployed])
    Security --> Done`} caption="Software studio stages. Edges are input dependencies declared in each stage's STAGE.md. Review gates enforce human-in-the-loop at every stage except Operations." />

Inception is the root — every other stage reads its discovery artifacts. Design and Product both feed Development. Operations and Security need code and architecture from Development. Security is the only stage that runs autonomously on every input, because threat modeling doesn't need elaboration turns.
