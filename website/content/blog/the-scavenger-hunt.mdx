...
Every studio is a named lifecycle template — a DAG of stages, each of which runs its own phase FSM. The default software studio has six stages in strict order: **inception → design → product → development → operations → security**. Every stage walks the same phase sequence: `pending → elaborate → execute → review → gate`. The orchestrator enforces the transitions; the agent can only move forward when the preconditions are actually met, not when it thinks they are.
...
The full state machine — six stages × five phases, every gate type, mode resolution, `/haiku:revisit` back-edges — lives in `packages/haiku/src/orchestrator.ts`. A diagram of it is a post for another day.
