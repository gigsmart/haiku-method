...
- **No session-ID-to-unit mapping today.** `CLAUDE_SESSION_ID` is read in hook files (`hooks/inject-state-file.ts:48`, `hooks/inject-context.ts:69`, `hooks/context-monitor.ts:29`, `sentry.ts:27-28`) but is never recorded on units or bolts. The `state-tools.ts` boundary handlers can read `process.env.CLAUDE_SESSION_ID` directly at bolt-boundary time without new plumbing. Resolved in `knowledge/CORRELATION_STRATEGY.md` and the stage rollup.
