...
status: pending
quality_gates:
  - Request/response schemas for all HTTP endpoints with field types and required/optional
...
- Note: question images currently use absolute paths by index — resolve to relative paths at session creation time so they fit this consolidated route
