...
- [x] **New ordering regression test:** a fixture with `{ status: "completed", archived: true }` in frontmatter MUST return `action: "complete"` from `runNext`, NOT `action: "error"`. This proves the new field-based check is inserted AFTER the `status === "completed"` branch, not before it. Without this test, a future refactor could reorder the guards and silently break completion.
