...
- [ ] `bun test` in `packages/haiku/` passes. The three existing `haiku_intent_list` tests in `test/state-tools-handlers.test.mjs:265-291` continue to pass unchanged (no fixture intents set `archived: true`, so default-filter behavior is a no-op for them). The `haiku_intent_list requires no arguments` server-tools assertion in `test/server-tools.test.mjs` still holds because `include_archived` is optional.
