...
hat_started_at: '2026-04-14T21:59:05Z'
completed_at: '2026-04-14T22:05:31Z'
---
...
- [x] No regression on existing tests: `orchestrator.test.mjs:169-173`, `:191-196`, and `:198-204` all pass unchanged.
- [x] New ordering regression test: a fixture with `{ status: "completed", archived: true }` MUST return `action: "complete"` from `runNext`, NOT `action: "error"`.
- [x] `bun run typecheck` and `bun test` both green. Manual end-to-end: archive → list (hidden) → run_next (error with "unarchive") → unarchive → list (visible) → run_next (advances).
