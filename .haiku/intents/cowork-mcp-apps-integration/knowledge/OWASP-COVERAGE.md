...
| A06 — Vulnerable and Outdated Components | Applicable | Direct deps (`zod`, `@modelcontextprotocol/sdk`) have no known CVEs. `npm audit` reports 3 findings in transitive chain: `axios <=1.14.0` (critical/high, via `localtunnel`) and `follow-redirects` (moderate). These are reachable only via `tunnel.ts` (HTTP review path), not the MCP Apps path. VUL-007 documents accepted risk with isolation justification. | Covered (accepted risk VUL-007) |
