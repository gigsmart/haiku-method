...
| A06 — Vulnerable and Outdated Components | Applicable | Direct deps: `zod ^3.23.0`, `@modelcontextprotocol/sdk ^1.28.0`, `@modelcontextprotocol/ext-apps` (review-app). No known CVEs in these versions as of 2026-04-15. Dependency scanning via standard `npm audit` in CI. `package.json` uses `^` semver ranges — patch updates applied automatically. | Covered |
...
- **0 Gaps**
- Accepted risks: VUL-004 (sequential request IDs, LOW, requires XSS prerequisite), VUL-005 (console.log, LOW, no secret exposure)
