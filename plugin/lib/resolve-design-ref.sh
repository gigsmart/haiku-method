...
_resolve_design_ref_field() {
  ...
  if [[ "$design_ref" =~ ^[a-z]+:// ]]; then
    echo "haiku: resolve-design-ref: provider URI not yet supported: $design_ref" >&2
    echo "haiku: resolve-design-ref: falling through to next priority level" >&2
    return 1
  fi
...
hku_resolve_design_ref() {
  ...
  local output_dir="$repo_root/.haiku/$intent_slug/screenshots/$unit_slug"
