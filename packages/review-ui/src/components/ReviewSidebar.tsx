...
export function ReviewSidebar({
	...
	async function doApprove() {
		...
		try {
			...
			await transport.submitDecision("approved", "", annotations)
	...
	async function handleRequestChanges() {
		...
		try {
			...
			await transport.submitDecision("changes_requested", feedback, annotations)
			tryCloseTab(setShowClose, {
				url: `/review/${sessionId}/decide`,
	...
	async function doExternalReview() {
		...
		try {
			const annotations = getAnnotations()
			await transport.submitDecision(
				"external_review",
		...
		<aside className="hidden md:flex w-80 lg:w-96 shrink-0 sticky top-16 h-[calc(100vh-4rem)] flex-col bg-white dark:bg-stone-900 border-l border-stone-200 dark:border-stone-700">
			...
			<div className="flex-1 overflow-y-auto p-3 space-y-2">
				...
				{comments.map((c) => (
					<div
						key={c.id}
						// biome-ignore lint/a11y/useSemanticElements: card contains interactive edit/delete buttons; nesting a <button> inside another is invalid HTML
						role="button"
						tabIndex={0}
					...
					>
						...
						{editingId === c.id ? (
							// biome-ignore lint/a11y/useKeyWithClickEvents: wrapper only stops click propagation; inner textarea owns keyboard interaction
