...
import { useReviewTransport } from "../context"
import { tryCloseTab } from "../hooks/tryCloseTab"
...
export function DesignPicker({ session, sessionId }: Props) {
	const transport = useReviewTransport()
	...
	async function handleSubmit() {
		...
		try {
			await transport.submitDirection(selectedArchetype, paramValues)
			setResult({
				success: true,
		...
		<>
			...
			<Card>
				...
				>
					{archetypes.map((arch) => {
						...
						return (
							// biome-ignore lint/a11y/useKeyWithClickEvents: parent radiogroup above owns keyboard navigation (ArrowLeft/Right/Up/Down)
							<div
								key={arch.name}
								// biome-ignore lint/a11y/useSemanticElements: custom radio-card composite — native <input type="radio"> cannot contain an iframe preview
								role="radio"
			...
			{previewModal && (
				<div
					...
					// biome-ignore lint/a11y/useSemanticElements: native <dialog> + scroll-lock would be a separate refactor; this replicates an ad-hoc modal
					role="dialog"
					aria-modal="true"
					aria-label={`Full size preview: ${previewModal.name}`}
				>
					{/* biome-ignore lint/a11y/useKeyWithClickEvents: inner panel only stops click propagation; Escape is handled on the backdrop above */}
					<div
