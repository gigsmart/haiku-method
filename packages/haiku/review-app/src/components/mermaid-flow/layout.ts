...
function estimateNodeSize(label: string, shape: string): { width: number; height: number } {
  ...
  const targetWidth = Math.max(NODE_MIN_WIDTH, Math.min(NODE_MAX_WIDTH, longest * CHAR_WIDTH + H_PADDING))
  const charsPerLine = Math.max(10, Math.floor((targetWidth - H_PADDING) / CHAR_WIDTH))
  let totalLines = 0
  for (const line of rawLines) totalLines += Math.max(1, Math.ceil(line.length / charsPerLine))
...
export async function layoutFlow(parsed: ParsedFlow): Promise<{ nodes: Node[]; edges: Edge[] }> {
  const childrenByParent = new Map<string | null, Array<{ kind: "node" | "group"; id: string; label: string; shape: string }>>()

  const push = (parentId: string | null, entry: { kind: "node" | "group"; id: string; label: string; shape: string }) => {
    const list = childrenByParent.get(parentId) ?? []
    list.push(entry)
    childrenByParent.set(parentId, list)
  }

  for (const g of parsed.groups) push(g.parentId, { kind: "group", id: g.id, label: g.label, shape: "group" })
  for (const n of parsed.nodes) push(n.parentId, { kind: "node", id: n.id, label: n.label, shape: n.shape })

  const groupDir = new Map<string, string>()
  for (const g of parsed.groups) groupDir.set(g.id, g.direction)

  function buildElkNode(parentId: string | null): Array<Record<string, unknown>> {
  ...
  function walk(nodes: Array<Record<string, unknown>>, parentId: string | undefined, offsetX: number, offsetY: number): void {
    for (const n of nodes) {
      ...
      const isGroup = Array.isArray(n.children) && (n.children as unknown[]).length > 0
  ...
  walk((result.children ?? []) as Array<Record<string, unknown>>, undefined, 0, 0)
