...
async function importPanel() {
  const { BottomSheetDecisionPanelReview } = await import("../BottomSheetDecisionPanelReview");
...
describe("BottomSheetDecisionPanelReview — keyboard shortcuts", () => {
  it("pressing '1' triggers approve", async () => {
    ...
    expect(mockSubmit).toHaveBeenCalledWith("sid-ks", "approved", expect.any(String), undefined, undefined);
    unmount();
  });
  ...
  it("pressing '2' triggers changes_requested", async () => {
    ...
    expect(mockSubmit).toHaveBeenCalledWith("sid-ks2", "changes_requested", expect.any(String), undefined, undefined);
    unmount();
  });
  ...
  it("pressing '3' triggers external_review", async () => {
    ...
    expect(mockSubmit).toHaveBeenCalledWith("sid-ks3", "external_review", expect.any(String), undefined, undefined);
    unmount();
  });
});
...
describe("No window.close in iframe components (criterion 11)", () => {
  it("BottomSheetDecisionPanelReview has no tryCloseTab calls", async () => {
    ...
    expect(typeof (window as typeof window & { tryCloseTab?: unknown }).tryCloseTab).toBe("undefined");
  });
});
