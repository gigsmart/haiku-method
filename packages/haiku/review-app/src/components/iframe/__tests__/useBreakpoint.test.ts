...
let lastObserverInstance: { observe: ReturnType<typeof vi.fn>; disconnect: ReturnType<typeof vi.fn>; unobserve: ReturnType<typeof vi.fn> } | null = null;

class MockResizeObserver {
  constructor(callback: ResizeObserverCallback) {
    observerCallback = callback;
  }
  observe = vi.fn((el: Element) => { lastObserved = el; });
  disconnect = vi.fn();
  unobserve = vi.fn();
}
...
function fireResize(width: number) {
  if (observerCallback && lastObserved) {
    act(() => {
      observerCallback!(
        [{ contentRect: { width } as DOMRectReadOnly, target: lastObserved! } as ResizeObserverEntry],
        {} as ResizeObserver,
      );
    });
  }
...
describe("useBreakpoint", () => {
  ...
  it("returns narrow when ResizeObserver fires width 400", () => {
    ...
    const { result } = renderHook(() => useBreakpoint(ref as React.RefObject<HTMLElement>));
    fireResize(400);
  ...
  it("disconnects the observer on unmount", () => {
    ...
    document.body.appendChild(el);
    const ref = { current: el };
    const { unmount } = renderHook(() => useBreakpoint(ref as React.RefObject<HTMLElement>));
