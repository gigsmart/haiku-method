 ...
 */
export function NegotiationErrorScreen({ errorCode, sessionId, onRetry }: Props) {
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
      ...
      >
        ...
        <div className="flex justify-center">
          <div className="w-12 h-12 rounded-full bg-red-950/40 flex items-center justify-center" aria-hidden="true">
        ...
        <div className="text-center">
          <h2 className="text-base font-semibold text-stone-100 mb-1">Connection Failed</h2>
        ...
        >
          {retrying ? (
            <span className="flex items-center justify-center gap-2">
              <span className="w-4 h-4 border-2 border-stone-400 border-t-stone-50 rounded-full animate-spin" aria-hidden="true" />
              Retrying…
            </span>
          ) : retryFailed ? "Retry failed — try again" : "Retry"}
        </button>
        ...
        {retryFailed && (
          <div className="bg-stone-950 border border-stone-700 rounded-lg p-3 flex flex-col gap-2">
            <p className="text-xs text-stone-400">Copy the session ID to report this issue:</p>
