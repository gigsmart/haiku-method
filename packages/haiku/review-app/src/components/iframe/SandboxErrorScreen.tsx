		...
		<div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
			...
			>
				...
				<div className="flex justify-center">
					<div
						className="w-12 h-12 rounded-full bg-amber-950/40 flex items-center justify-center"
						aria-hidden="true"
					>
						<svg
				...
				<div className="text-center">
					...
					<p className="text-sm text-stone-300">
						The iframe sandbox blocked:{" "}
						<code className="font-mono text-amber-400">{feature}</code>
					</p>
				</div>
				...
				<details
					open={disclosureOpen}
					onToggle={(e) =>
						setDisclosureOpen((e.currentTarget as HTMLDetailsElement).open)
					}
				...
				>
					...
					>
						...
						<span
							aria-hidden="true"
							className={`text-xs transition-transform ${disclosureOpen ? "rotate-180" : ""}`}
						>
							▼
						</span>
					</summary>
					<div className="px-3 pb-3 pt-1 text-xs text-stone-400 leading-relaxed">
						<p>
							The host application runs this review UI inside a sandboxed
							iframe. The sandbox policy restricts certain browser APIs (like
							clipboard access) to prevent cross-frame data leaks.
						</p>
						<p className="mt-2">
							To resolve this, ask the host application to include{" "}
							<code className="font-mono text-stone-300">allow-{feature}</code>{" "}
							in its iframe sandbox attribute.
						</p>
					</div>
				</details>
			</div>
