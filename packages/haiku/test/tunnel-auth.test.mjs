...
async function run() {
	...
	await test(
		"GET /api/session/:sid without Authorization returns 401 (missing_token)",
		async () => {
	...
	await test(
		"GET /api/session/:sid with malformed Authorization returns 401 (malformed)",
		async () => {
			const res = await fetch(`${baseUrl}/api/session/${session.session_id}`, {
	...
	await test(
		"GET /api/session/:sid with bad signature returns 401 (bad_signature)",
		async () => {
	...
	await test(
		"GET /api/session/:sid with expired token returns 401 (expired)",
		async () => {
			const token = mintToken(session.session_id, {
	...
	await test(
		"GET /api/session/:sid with tunnel-mismatch token returns 401 (tunnel_mismatch)",
		async () => {
			const token = mintToken(session.session_id, {
	...
	await test(
		"GET /api/session/:sid with wrong-sid token returns 401 (sid_mismatch)",
		async () => {
	...
	await test(
		"GET /api/session/:sid with valid Authorization returns 200",
		async () => {
			const token = mintToken(session.session_id)
			const res = await fetch(`${baseUrl}/api/session/${session.session_id}`, {
				headers: { Authorization: `Bearer ${token}` },
	...
	await test(
		"POST /review/:sid/decide without token returns 401",
		async () => {
			const res = await fetch(
				`${baseUrl}/review/${session.session_id}/decide`,
				{
					method: "POST",
	...
	await test(
		"POST /api/revisit/:sid without token returns 401",
		async () => {
	...
	await test(
		"GET /api/review/current without token returns 401",
		async () => {
	...
	await test(
		"GET /api/review/current with any valid tunnel-bound token returns non-401",
		async () => {
			const token = mintToken(session.session_id)
			const res = await fetch(`${baseUrl}/api/review/current`, {
				headers: { Authorization: `Bearer ${token}` },
	...
	await test(
		"GET /api/feedback/:intent/:stage without token returns 401",
		async () => {
			const res = await fetch(
	...
	await test(
		"POST /api/feedback/:intent/:stage without token returns 401 (tunnel gate fires before FB-20)",
		async () => {
			const res = await fetch(
				`${baseUrl}/api/feedback/${intentSlug}/${stageName}`,
				{
	...
	await test(
		"GET /files/:sid/path without ?t returns 401",
		async () => {
	...
	await test(
		"GET /files/:sid/path with bad ?t returns 401",
		async () => {
			const res = await fetch(
	...
	await test(
		"GET /mockups/:sid/:path without ?t returns 401",
		async () => {
			const res = await fetch(
				`${baseUrl}/mockups/${session.session_id}/m.png`,
			)
			assert.strictEqual(res.status, 401)
		},
	)
	...
	await test(
		"GET /wireframe/:sid/:path without ?t returns 401",
		async () => {
			const res = await fetch(
				`${baseUrl}/wireframe/${session.session_id}/w.png`,
			)
			assert.strictEqual(res.status, 401)
		},
	)
	...
	await test(
		"GET /stage-artifacts/:sid/:path without ?t returns 401",
		async () => {
			const res = await fetch(
	...
	await test(
		"GET /question-image/:sid/:index without ?t returns 401",
		async () => {
			const res = await fetch(
				`${baseUrl}/question-image/${session.session_id}/0`,
			)
			assert.strictEqual(res.status, 401)
		},
	)
	...
	await test(
		"GET /files/:sid/path with valid ?t clears the auth gate",
		async () => {
			const token = mintToken(session.session_id)
			const res = await fetch(
	...
	await test(
		"GET /review/current (SPA shell) does NOT require a token",
		async () => {
	...
	await test(
		"GET /review/:sid (SPA shell) does NOT require a token",
		async () => {
	...
	await test(
		"GET /health never requires a token (tunnel keepalive)",
		async () => {
