...
registerPrompt({
	...
	handler: async () => ({
		messages: [
			{ role: "user", content: { type: "text", text: "minimal response" } },
		],
	}),
})
		...
		messages: [
			{
				...
				content: {
					type: "text",
					text: `intent=${args.intent}, stage=${args.stage || "none"}`,
				},
			},
		],
	}),
...
registerPrompt({
	...
	handler: async (args) => ({
		messages: [
			{ role: "user", content: { type: "text", text: `color=${args.color}` } },
		],
	}),
})
...
try {
	...
	test("returns array of prompts", () => {
		...
		assert.ok(
			prompts.length >= 4,
			`Expected at least 4 prompts, got ${prompts.length}`,
		)
	})
	...
	await testAsync("passes arguments to handler", async () => {
		...
		assert.strictEqual(
			...
			"intent=my-feature, stage=none",
		)
	})

	await testAsync("passes all arguments to handler", async () => {
		const result = await getPrompt("test:with-args", {
			intent: "feat",
			stage: "inception",
		})
		assert.strictEqual(
			result.messages[0].content.text,
			"intent=feat, stage=inception",
		)
	})
	...
	await testAsync("throws McpError for missing required argument", async () => {
		try {
			...
			assert.ok(
				...
				`Expected 'Missing required argument', got: ${e.message}`,
			)
		}
	})
	...
	await testAsync(
		"returns completions for argument with completer",
		async () => {
			const result = await completeArgument({
			...
			assert.ok(
				result.completion.values.length === 6,
				`Expected 6 colors, got ${result.completion.values.length}`,
			)
		},
	)
