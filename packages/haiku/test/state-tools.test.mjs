...
// Write stage state
writeFileSync(
	join(haikuDir, "stages", "inception", "state.json"),
	JSON.stringify({
		stage: "inception",
		status: "active",
		phase: "elaborate",
...
// Write units
writeFileSync(
	join(haikuDir, "stages", "inception", "units", "unit-01-discovery.md"),
	`---
...
writeFileSync(
	join(haikuDir, "stages", "inception", "units", "unit-02-elaborate.md"),
	`---
...
// Write knowledge
writeFileSync(
	join(haikuDir, "knowledge", "discovery.md"),
	"# Discovery Document\n\nContent here.",
)
...
test("get stage phase from JSON", () => {
	const result = run(
		`get "${join(haikuDir, "stages/inception/state.json")}" phase`,
	)
...
test("set stage status to completed", () => {
	...
	const result = run(
		`get "${join(haikuDir, "stages/inception/state.json")}" status`,
	)
...
test("get unit status", () => {
	const result = run(
		`get "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" status`,
	)
	assert.strictEqual(result, "active")
})
...
test("get unit bolt", () => {
	const result = run(
		`get "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" bolt`,
	)
...
test("get unit hat", () => {
	const result = run(
		`get "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" hat`,
	)
...
test("set unit hat to elaborator", () => {
	run(
		`set "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" hat elaborator`,
	)
	const result = run(
		`get "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" hat`,
	)
...
test("increment unit bolt", () => {
	run(
		`set "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" bolt 3`,
	)
	const result = run(
		`get "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" bolt`,
	)
...
test("set unit status to completed", () => {
	run(
		`set "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" status completed`,
	)
	const result = run(
		`get "${join(haikuDir, "stages/inception/units/unit-01-discovery.md")}" status`,
	)
...
test("set preserves body content", () => {
	run(
		`set "${join(haikuDir, "stages/inception/units/unit-02-elaborate.md")}" status active`,
	)
