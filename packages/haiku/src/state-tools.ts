...
export const stateToolDefs = [
	...
	},
	{
		name: "haiku_repair",
		description:
			"Scan intents for metadata issues (missing fields, invalid studios, stages mismatch, legacy fields, unit naming). Returns a diagnostic report with fixes.",
		inputSchema: {
			type: "object" as const,
			properties: {
				intent: {
					type: "string",
					description: "Specific intent slug to scan, or omit to scan all",
				},
			},
		},
	},
...
export function handleStateTool(
	...
	switch (name) {
		...
		case "haiku_studio_get": {
			...
			let studioFile = ""
			try {
				studioFile = join(findHaikuRoot(), "studios", studioName, "STUDIO.md")
			} catch {
				/* */
			}
		...
		case "haiku_repair": {
			...
			const repairSearchPaths = [
				join(root, "studios"),
				join(repairPluginRoot, "studios"),
			]
			for (const base of repairSearchPaths) {
				...
				for (const d of readdirSync(base, { withFileTypes: true })) {
					...
					const { data: stData } = parseFrontmatter(
						readFileSync(studioMd, "utf8"),
					)
			...
			if (!existsSync(repairIntentsDir))
				return text("No intents directory found.")
			...
			if (repairIntentArg) {
				if (/[/\\]|\.\./.test(repairIntentArg))
					return text(`Invalid intent slug: "${repairIntentArg}"`)
				...
				repairSlugs = readdirSync(repairIntentsDir, { withFileTypes: true })
					.filter(
						(d) =>
							d.isDirectory() &&
							existsSync(join(repairIntentsDir, d.name, "intent.md")),
					)
					.map((d) => d.name)
			}
			...
			for (const slug of repairSlugs) {
				...
				if (
					...
					issues.push({
						...
						fix: "Add `title` field with a descriptive name",
					})
				}
				...
				if (
					...
					repairStudio &&
					repairStudioMap.has(repairStudio)
				) {
				...
				if (Array.isArray(repairStages) && repairStages.length > 0) {
					...
					const repairActiveStage = repairData.active_stage as
						| string
						| undefined
					...
					for (const stageName of repairStages as string[]) {
						...
						if (existsSync(repairStateFile)) {
							...
							if (
								state.status &&
								!validStatuses.includes(state.status as string)
							) {
								issues.push({
									intent: slug,
							...
							const activeIdx = (repairStages as string[]).indexOf(
								repairActiveStage,
							)
				...
				if (Array.isArray(repairStages)) {
					for (const stageName of repairStages as string[]) {
						...
						for (const f of readdirSync(repairUnitsDir, {
							withFileTypes: true,
						})) {
					...
					for (const stageName of repairStages as string[]) {
						...
						if (repairStudio) {
							...
							for (const base of repairSearchPaths) {
								...
								if (
									Array.isArray(stageData.inputs) &&
									...
									for (const base of repairSearchPaths) {
										for (const kind of ["discovery", "outputs"] as const) {
											...
											for (const f of readdirSync(artifactDir).filter((af) =>
												...
												const aName =
													(aData.name as string) || f.replace(/\.md$/, "")
												const wanted =
												...
												if (existsSync(absPath))
													existingUpstreamPaths.push(relPath)
											}
										}
									}
								}
						...
						for (const f of readdirSync(repairUnitsDir, {
							...
							if (["complete", "skipped", "failed"].includes(unitStatus))
								continue
							const unitInputs =
								(unitData.inputs as string[]) ||
								(unitData.refs as string[]) ||
								[]
