		...
		case "start_units": {
			...
			for (const unitName of units) {
				...
				unitInstrLines.push(
					...
					"",
					"**Autonomy (CRITICAL):** You are one of a parallel wave — execute your unit without asking the user for confirmation. Do **NOT** ask which unit runs first, whether to continue, or for approval after a summary. The FSM coordinates the wave; you coordinate the work. Only stop to ask when the task is genuinely ambiguous or a required input is missing.",
					"",
					"**When you genuinely must ask** (unblock-only):",
					"- Use `AskUserQuestion` with an `options[]` array when alternatives are known — never plain-text option lists.",
